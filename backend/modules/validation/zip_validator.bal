...
public isolated function validateZipFile(byte[] zipData, string filename) returns ZipValidationResult {
    // string[] errors = [];
    logging:LogRecord logRecord = { level: "DEBUG", message: "ZIP validation started" };
    ...
    if !basicResult.isValid {
        ...
        logRecord = {
            level: "ERROR",
            message: "Basic ZIP structure validation failed"
        };
    ...
    logRecord = { level: "INFO", message: "1) Basic ZIP structure validation passed" };
    logging:log(logRecord);
    ...
    if !filenameResult.isValid {
        ...
        logRecord = { level: "ERROR", message: "Filename validation failed" };
        logging:log(logRecord);
    ...
    logRecord = { level: "INFO", message: "2) Filename validation passed" };
    logging:log(logRecord);
    ...
    if !structureResult.isValid {
        ...
        logRecord = { level: "ERROR", message: "ZIP content security analysis failed" };
        logging:log(logRecord);
    ...
    logRecord = { level: "INFO", message: "3) ZIP content security analysis passed" };
    logging:log(logRecord);
...
isolated function analyzeZipStructure(byte[] zipData) returns ZipValidationResult {
    ...
    while offset < zipData.length() - 30 {
        ...
        if hasZipSignature(zipData, offset, ZIP_CENTRAL_HEADER_SIGNATURE) {
            ...
            if uncompressedSize > MAX_UNCOMPRESSED_SIZE_MB * 1024 * 1024 {
