...
describe('MenuBar Component (TS)', () => {
  const makeDefaultProps = (overrides: Partial<MenuBarProps> = {}): MenuBarProps => {
  ...
  test('renders with different activeKey values', () => {
    const { rerender } = render(<MenuBar {...makeDefaultProps({ activeKey: 'microapp' })} />);
  ...
  test('handles undefined activeKey', () => {
    render(<MenuBar {...makeDefaultProps({ activeKey: undefined as unknown as string })} />);
    expect(screen.getByTestId('menu-item-microapp')).not.toHaveClass('selected');
