...
import { ConfirmDialog, EditMicroAppDialog, AddVersionDialog } from '../components';
...
const MicroApps = () => {
  ...
  const [editDialog, setEditDialog] = useState<{ open: boolean; microApp?: MicroApp }>({
    open: false,
  });
  const [versionDialog, setVersionDialog] = useState<{ open: boolean; microApp?: MicroApp }>({
    open: false,
  });
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; appId: string; appName: string }>({
    open: false,
    appId: '',
    appName: '',
  });
  const [menuAnchor, setMenuAnchor] = useState<{ element: HTMLElement | null; microApp: MicroApp | null }>({
    element: null,
    microApp: null,
  });
  ...
  const handleMenuOpen = (event: React.MouseEvent<HTMLElement>, microApp: MicroApp) => {
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
                ...
                <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  <Box display="flex" alignItems="center" mb={2}>
                    ...
                    <Box flexGrow={1}>
                      ...
                      <Box display="flex" gap={1} flexWrap="wrap">
                        <Chip
                          label={app.isMandatory === 1 ? 'Mandatory' : 'Optional'}
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary" display="block" mb={0.5}>
                      Roles: {app.roles.length > 0 ? app.roles.map(role => role.role).join(', ') : 'None'}
                    </Typography>
                  ...
                  <Box mt={2}>
                    <Button
                      ...
                      endIcon={expandedCards.has(app.appId) ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                      sx={{ textTransform: 'none', mb: 1 }}
                    >
                      {expandedCards.has(app.appId) ? 'Hide' : 'Show'} Versions ({app.versions.length})
                    </Button>
      ...
      >
        ...
        <MenuItem 
          onClick={() => {
            if (menuAnchor.microApp) {
              handleDeleteClick(menuAnchor.microApp.appId, menuAnchor.microApp.name);
            }
            handleMenuClose();
          }}
          sx={{ color: 'error.main' }}
