...
import { validateZipFile, isZipExtension, formatFileSize, getMaxFileSize } from '../zipValidator';
...
describe('zipValidator', () => {
  ...
  describe('validateZipFile', () => {
    ...
    it('should reject file that is too small', async () => {
      const file = new File([new Uint8Array(10)], 'test.zip', { type: 'application/zip' });
      const result = await validateZipFile(file);
      expect(result.valid).toBe(false);
    ...
    it('should accept valid ZIP with correct signature', async () => {
      ...
      const header = new Uint8Array([
        ...
        0x50, 0x4b, 0x05, 0x06, // End signature
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      ]);
