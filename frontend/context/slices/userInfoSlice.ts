...
const userInfoSlice = createSlice({
  ...
  reducers: {
    setUserInfo(state, action: PayloadAction<UserInfo>) {
      ...
      // Save to AsyncStorage whenever userInfo is updated
      if (state.userInfo) {
        AsyncStorage.setItem(USER_INFO, JSON.stringify(state.userInfo))
          .then(() => {
            console.log("UserInfo saved to AsyncStorage successfully");
          })
          .catch((error) => {
    ...
    clearUserInfo(state) {
      state.userInfo = null;
      state.error = null;
      // Remove from AsyncStorage when clearing userInfo
      AsyncStorage.removeItem(USER_INFO)
        .then(() => {
          console.log("UserInfo cleared from AsyncStorage successfully");
        })
        .catch((error) => {
          console.error("Failed to clear userInfo from AsyncStorage:", error);
        });
    },
  },
